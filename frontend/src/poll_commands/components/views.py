...
class MakerButtons(discord.ui.View):
    ...
    def __init__(
        self, poll_name: str, guild_id: int, maker: discord.User, question_dict, *items
    ):
        """
    ...
    @discord.ui.button(
        label="Add MC Question", row=0, style=discord.ButtonStyle.primary
    )
    async def mc_button_callback(
    ...
    async def remove_button_callback(
        ...
        if interaction.user != self.maker:
            ...
            remove_modal = RemoveModal(
                title="Remove Question Form",
                question_list=self.question_dict[
                    (interaction.guild_id, self.poll_name)
                ],
            )
    ...
    @discord.ui.button(
        label="Cancel Creation", row=0, style=discord.ButtonStyle.primary
    )
    async def cancel_button_callback(
        ...
        await interaction.response.send_message(
            content=f"Cancelled Creation of {self.poll_name}", ephemeral=True
        )
        await interaction.message.delete()

    @discord.ui.button(label="Finalize Poll", row=1, style=discord.ButtonStyle.green)
    async def finalize_button_callback(
        self, button: discord.Button, interaction: discord.Interaction
    ):
        """
        ...
        if interaction.user != self.maker:
            ...
            await interaction.response.send_message(
                content=f"You can't make an empty poll", ephemeral=True
            )
        else:
            await interaction.response.send_message(
                view=PollButtons(
...
class PollButtons(discord.ui.View):
    ...
    def as_selector_options(self):
        ...
        """
        options = []
        answers = self.question_dict[(self.guild_id, self.poll_name)][
            self.question_num
        ].options.split(",")
        for i in answers:
    ...
    def make_select(self):
        ...
        answer_selector = discord.ui.Select(
            ...
            placeholder=self.question_dict[(self.guild_id, self.poll_name)][
                self.question_num
            ].question,
        )
        ...
        async def answer_select(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(
                content=f"{interaction.user.name} voted for: {choice}", ephemeral=True
            )
        ...
        for question in self.results.keys():
            ...
            for option in self.results[question].keys():
                result_embed.add_field(
                    name=option, value=self.results[question][option]
                )
    ...
    async def next_button_callback(
        self, button: discord.Button, interaction: discord.interactions
    ):
        """
        ...
        if self.question_num >= len(question_list) - 1:
            ...
            BackendClient().save_poll_results(
